...
import com.milko.awsfilestorage.exception.AuthException;
...
public class SecurityService {
    ...
    private TokenDetails generateToken(UserDto userDto){
        Map<String, Object> claims = new HashMap<>(){{
           put("role", userDto.getRole());
           put("username", userDto.getUsername());
        }};
        return generateToken(claims, userDto.getId().toString());
    }
        ...
        return userServiceImpl.getUserByUsername(username)
                .flatMap(userDto -> {
                    if (userDto.getStatus().equals(Status.DELETED)){
                        return Mono.error(new AuthException("Account deleted", "ACCOUNT_DELETED"));
                    }
                    if (!passwordEncoder.matches(password, userDto.getPassword())){
                        return Mono.error(new AuthException("Invalid password", "INVALID_PASSWORD"));
                    }

                    return Mono.just(generateToken(userDto).toBuilder().userId(userDto.getId()).build());
                })
